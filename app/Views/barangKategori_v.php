...
<div class="card-box">
    <?= form_open('cari', 'class="form-inline d-flex justify-content-end w-100"'); ?>
    <?= form_hidden('id_kategori', $idKategori); ?>
    <input name="txt-cari" class="form-control mr-0" type="text" placeholder="Cari nama barang ..." aria-label="Cari..">
    <button type="submit" class="btn btn-small btn-light rounded-right"><i class="icon-magnifier" aria-hidden="true"></i></button>
    <?= form_close(); ?>
</div>

<div class="row row-cols-2 row-cols-sm-3 row-cols-md-6">
    <?php foreach ($dtMain as $brg) : ?>
